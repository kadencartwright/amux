...
The TUI sidebar hotkey SHALL be configurable via the `sidebar_toggle_key` configuration option, defaulting to "S" (uppercase S).
...
- **THEN** tmux binds `Prefix + S` to toggle the sidebar
...
- **AND** the default `Prefix + S` is NOT bound
...
The TUI sidebar SHALL support showing the overlay again when the user presses the tmux prefix hotkey (e.g., `Prefix + S`) while in pass-through mode.

#### Scenario: User shows the sidebar via hotkey
- **GIVEN** the sidebar is in pass-through mode (hidden)
- **WHEN** the user presses the configured tmux hotkey (e.g., `Ctrl+A S`)
...
- **WHEN** the user presses the tmux prefix hotkey (e.g., `Ctrl+A S`)
